...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 BG) in its wallet.
...
with the glbitcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a glbitcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Bitcoin Global node application logic. For custom behaviour, subclass the
...
Basic code to support P2P connectivity to a glbitcoind.
