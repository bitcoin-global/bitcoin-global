...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for i in range(1, numTxes + 1):
            ...
            sig = self.coinbase_key.sign_ecdsa(sighash) + bytes(bytearray([SIGHASH_ALL|SIGHASH_FORKID]))
    ...
    def sign_tx(self, tx, spend_tx):
        ...
        if (scriptPubKey[0] == OP_TRUE):  # an anyone-can-spend
            ...
            return
        (sighash, err) = SignatureHash(spend_tx.vout[0].scriptPubKey, tx, 0, SIGHASH_ALL|SIGHASH_FORKID)
        tx.vin[0].scriptSig = CScript([self.coinbase_key.sign_ecdsa(sighash) + bytes(bytearray([SIGHASH_ALL|SIGHASH_FORKID]))])
