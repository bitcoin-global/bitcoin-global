...
const char * const BITCOIN_CONF_FILENAME = "bitglob.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitglobd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitglob-tx using stdin
        std::string val;
...
fs::path GetDefaultDataDir()
{
    ...
    // Mac: ~/Library/Application Support/bitglobal
    // Unix: ~/.bitglobal
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitglobal";
#else
    fs::path pathRet;
...
std::string CopyrightHolders(const std::string& strPrefix, const std::string& defaultPrefix)
{
    ...
    if (copyright_devs.find("Bitcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + defaultPrefix + "The Bitcoin Core developers";
    }
