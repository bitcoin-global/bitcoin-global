...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    for (uint32_t test_flags=0; test_flags < (1U << 17); test_flags += 1) {
        ...
        bool ret = CheckInputs(tx, state, &::ChainstateActive().CoinsTip(), test_flags | SCRIPT_FORKID_DISABLED, true, add_to_cache, txdata, nullptr);
        ...
        if (ret && add_to_cache) {
            ...
            BOOST_CHECK(CheckInputs(tx, state, &::ChainstateActive().CoinsTip(), test_flags | SCRIPT_FORKID_DISABLED, true, add_to_cache, txdata, &scriptchecks));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        BOOST_CHECK(!CheckInputs(CTransaction(spend_tx), state, &::ChainstateActive().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG | SCRIPT_FORKID_DISABLED, true, true, ptd_spend_tx, nullptr));

        // If we call again asking for scriptchecks (as happens in
        // ConnectBlock), we should add a script check object for this -- we're
        // not caching invalidity (if that changes, delete this test case).
        std::vector<CScriptCheck> scriptchecks;
        BOOST_CHECK(CheckInputs(CTransaction(spend_tx), state, &::ChainstateActive().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG | SCRIPT_FORKID_DISABLED, true, true, ptd_spend_tx, &scriptchecks));
    ...
    // for the same tx with a different witness.
    {
        ...
        BOOST_CHECK(ProduceSignature(keystore, MutableTransactionSignatureCreator(&valid_with_witness_tx, 0, 11*CENT, false, SIGHASH_ALL), spend_tx.vout[1].scriptPubKey, sigdata, true));
...

    {
        ...
        for (int i=0; i<2; ++i) {
            SignatureData sigdata;
            BOOST_CHECK(ProduceSignature(keystore, MutableTransactionSignatureCreator(&tx, i, 11*CENT, false, SIGHASH_ALL), spend_tx.vout[i].scriptPubKey, sigdata, true));
