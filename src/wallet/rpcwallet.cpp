    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getnewaddress",
                "\nReturns a new Bitcoin Global address for receiving payments.\n"
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Bitcoin Global address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bitcoin Global address");
    }
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoin Global address: ") + name_);
        }
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Bitcoin Global address or hex-encoded public key.\n"
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of bitcoin addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "bitcoin global address or hex-encoded public key"},
                        },
                        },
...
static UniValue loadwallet(const JSONRPCRequest& request)
{
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting bitglobd will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of bitcoin addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "bitcoin-global address"},
                        },
                    },
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoin Global address: ") + input.get_str());
            }
...
UniValue signrawtransactionwithwallet(const JSONRPCRequest& request)
{
    ...
    // Verify active forks
    bool no_forkid = !pwallet->chain().isBTGHardForkEnabledForCurrentBlock();

    // Parse the prevtxs array
    ParsePrevouts(request.params[1], nullptr, coins);

    return SignTransaction(mtx, pwallet, coins, request.params[2], no_forkid);
}
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
            ...
            "                         Must be at least 0.0001 BG per kB higher than the current transaction fee rate.\n"},
