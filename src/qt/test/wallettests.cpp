...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_bitglob-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_bitglob-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_bitglob-qt  # macOS
void TestGUI()
{
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: bitglobal:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_bitglob-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI();
