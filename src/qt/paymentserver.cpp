...
const QString BITCOIN_IPC_PREFIX("bitglobal:");
#ifdef ENABLE_BIP70
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the bitglobal: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitglobal: URI
        {
...
#endif
{
    ...
    // on Mac: sent when you click bitglobal: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start bitglobal: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling bitglobal: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bitglobal://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bitglobal://' is not a valid URI. Use 'bitglobal:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitglobal: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Bitcoin Global address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in bitglobal: URIs
