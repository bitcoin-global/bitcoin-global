...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitcoin Global address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no bitglobal: URI
    if(!uri.isValid() || uri.scheme() != QString("bitglobal"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitglobal:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open bitglob.conf with the associated application */
